...
class CodeGenerator extends BaseController
{
    ...
    public function generateAction()
    {
        ...
        if ($table == '' || !$this->validGenerateTable($table)) {
            ...
            $directories['controlleradmin'] = SITE_PATH . 'controllers'.DIRECTORY_SEPARATOR;
            $directories['controlleradmin'] .= '{{CONTROLLER_GROUP}}' . DIRECTORY_SEPARATOR;

            $directories['languageadmin'] = SITE_PATH . 'language' . DIRECTORY_SEPARATOR . $this->registry->langCode;
            $directories['languageadmin'] .= DIRECTORY_SEPARATOR . '{{CONTROLLER_GROUP}}' . DIRECTORY_SEPARATOR;

            $directories['templateadmin'] = SITE_PATH . $this->registry->smarty->template_dir . DIRECTORY_SEPARATOR;
            $directories['templateadmin'] .= '_controller'.DIRECTORY_SEPARATOR;
            $directories['templateadmin'] .= '{{CONTROLLER_GROUP}}' . DIRECTORY_SEPARATOR;

            $formData['directories'] = $directories;

            //template files
            $source_basedir = SITE_PATH . $this->registry->smarty->template_dir . '/' ;
            $source_basedir .= $this->registry->smartyController . 'generate_format/';
        ...
        if (isset($_POST['fsubmit'])) {
            ...
            $directories['languageadmin'] .= $this->registry->langCode . DIRECTORY_SEPARATOR;
            $directories['languageadmin'] .= strtolower($formData['fcontrollernamespace']). DIRECTORY_SEPARATOR;
            $directories['templateadmin'] = SITE_PATH . $this->registry->smarty->template_dir . DIRECTORY_SEPARATOR;
            $directories['templateadmin'] .= '_controller' . DIRECTORY_SEPARATOR;
            $directories['templateadmin'] .= strtolower($formData['fcontrollernamespace']). DIRECTORY_SEPARATOR;
            ...
            if ($this->generateValidate($formData, $error)) {
                ...
                if (isset($formData['fadmincontrollertoggler'])) {
                    ...
                    $leftMenuAppendString = '<li id="menu_'.strtolower($formData['fcontrollerclass']).'">
                        ...
                        strtolower($formData['fcontrollerclass']).'">
                        <i class="fa '.$formData['fcontrollericonclass'].'"></i> '.$formData['fmodule'].'s</a></li>';

                    $information[] = 'Add following line to
                        <code>/templates/default/_controller/admin/header_leftmenu.tpl</code>
                        to create menu item link to this controller:
                        <small><pre>' . htmlspecialchars($leftMenuAppendString) . '</pre></small>';

                    ///////////////////////////////
                    //CLASSMAP FOR CONTROLLER
                    $controllerClassmapAppendString = "'controller\\".strtolower($formData['fcontrollernamespace']).
                        "\\".strtolower($formData['fmodule'])."' => 'Controller' . DIRECTORY_SEPARATOR . '".
                        $formData['fcontrollernamespace']."' . DIRECTORY_SEPARATOR . '".$formData['fmodule'].".php',";

                    $information[] = '<a href="'.$this->registry->conf['rooturl_admin'].
                        $this->registry->controller . '/classmap" title="Generate Classmap Array" target="_blank">
                        <b>[CLICK HERE]</b></a> to generate new Classmap Array or just adding following line to Array
                        <code>$classmapList</code> declared in <code>/includes/classmap.php</code>
                        for mapping controller file name to class name: <small><pre>' .
                            htmlspecialchars($controllerClassmapAppendString) . '</pre></small>';

                    ///////////////////////////////
                    //PERMISSION FOR CONTROLLER
                    $controllerPermissionAppendString = "'".strtolower($formData['fcontrollerclass'])."_*',";
                    $information[] = 'Add following line to Array <code>$groupPermisson[...]</code>
                         declared in <code>/includes/permission.php</code> to grant permission for user group you want
                    ...
                    if (!file_exists($admintemplatedirectory)) {
                        if (!mkdir($admintemplatedirectory)) {
                            $error[] = 'Error while creating directory <code>'.
                                $directories['templateadmin'] . strtolower($adminmodule).
                                '</code> to store Admin template files.';
                        } else {
                            $savetplfiles = true;
                        }
                    } else {
                    ...
                    if ($savetplfiles) {
                        $destination['controlleradminindex'] = $directories['templateadmin'] . strtolower($adminmodule);
                        $destination['controlleradminindex'] .= DIRECTORY_SEPARATOR . 'index.tpl';

                        $destination['controlleradminadd'] = $directories['templateadmin'] . strtolower($adminmodule);
                        $destination['controlleradminadd'] .= DIRECTORY_SEPARATOR . 'add.tpl';

                        $destination['controlleradminedit'] = $directories['templateadmin'] . strtolower($adminmodule);
                        $destination['controlleradminedit'] .= DIRECTORY_SEPARATOR . 'edit.tpl';
                    }
    ...
    public function classmapAction()
    {
        ...
        $sourceBasedir = SITE_PATH
            . $this->registry->smarty->template_dir
            . DIRECTORY_SEPARATOR . $this->registry->smartyController
            . 'generate_format' . DIRECTORY_SEPARATOR;

        //Duyet thu muc controller de lay danh sach filename
        $controllerDir =  'Controller';
        if (is_readable($controllerDir)) {
            $fileListing = \Vendor\Litpi\Helper::directoryToArray($controllerDir, true);

            $classmapFiledata = file_get_contents($sourceBasedir . 'include_classmap.tpl');

            $classmapArray = array();
            foreach ($fileListing as $file) {
                $fileExt = \Vendor\Litpi\Helper::fileExtension($file);
                if ($fileExt == 'php') {
                    ...
                    for ($i = 0; $i < count($fileparts); $i++) {
                        if ($i != count($fileparts) - 1) {
                            ...
                            $classpathPart .= "'".$fileparts[$i]."' . " . 'DIRECTORY_SEPARATOR' . " . ";
                        } else {
                            // this is filename
        ...
        $this->registry->smarty->assign(array(  'pageTitle' => 'Classmap Generating',
                                                'contents'  => $contents));
        $this->registry->smarty->display($this->registry->smartyModule . 'index.tpl');
    }
    ...
    private function getSearchReplaceInfo($formData)
    {
        ...
        foreach ($formData['fprop'] as $k => $v) {
            $tmp .= "" . '    public $'.$v.' = '
                . $this->getDefaultValueFromColumnType($formData['ftypeshort'][$k]) . ';' . "\n";
        ...
        if (!empty($constantData)) {
            foreach ($constantData as $k => $constantGroups) {
                ...
                $filetemplatefunctionConstant = SITE_PATH . $this->registry->smarty->template_dir
                    . DIRECTORY_SEPARATOR . $this->registry->smartyController
                ...
                if (file_exists($filetemplatefunctionConstant)) {
                    ...
                    foreach ($constantGroups as $constantInfo) {
                        $st['{{PROPERTYCONSTANT_LIST}}'] .= "\$output[self::".$constantInfo[0]."] = '"
                            . $constantInfo[2]."';\n        ";
                        ...
                        $st['{{PROPERTYCONSTANT_CHECK}}'] .= "(\$this->".$formData['fprop'][$k]
                            . " == self::".$constantInfo[0]." && \$name == '".strtolower($constantInfo[2])."')";
                    }
                    ...
                    if ($tmpFunctionConstant != '') {
                        ...
                        $exceptionMsg = 'Can not OPEN tpl file for generate related methods
                            for constant property (Not found file tpl at '.$filetemplatefunctionConstant.').';
                        throw new Exception($exceptionMsg);
                    }
                } else {
                    $exceptionMsg = 'Can not FIND tpl file for generate related methods
                        for constant property (Not found file tpl at '.$filetemplatefunctionConstant.').';
                    throw new Exception($exceptionMsg);
                }
            ...
            if ($k != $formData['primaryfield']) {
                ...
                if ($formData['ftypeshort'][$k] == 'int') {
                    if (isset($formData['fipaddressable'][$k])) {
                        ...
                        $filetemplatefunctionGetMaxDisplayOrder = SITE_PATH
                            . $this->registry->smarty->template_dir . DIRECTORY_SEPARATOR
                        ...
                        if (file_exists($filetemplatefunctionGetMaxDisplayOrder)) {
                            ...
                            if ($tmpFunctionGetMaxDisplayOrder != '') {
                                ...
                                $exceptionMsg = 'Can not OPEN tpl file for generate function getMaxDisplayOrder()
                                    (Not found file tpl at '.$filetemplatefunctionGetMaxDisplayOrder.').';
                                throw new Exception($exceptionMsg);
                            }
                        } else {
                            $exceptionMsg = 'Can not FIND tpl file for generate function getMaxDisplayOrder()
                                (Not found file tpl at '.$filetemplatefunctionGetMaxDisplayOrder.').';
                            throw new Exception($exceptionMsg);
                        }
        ...
        foreach ($formData['fprop'] as $k => $v) {
            ...
            if (isset($formData['fipaddressable'][$k])) {
                ...
                $tmp2 .= '$my'.$formData['fmodule'].'->'
                    . $v . ' = (string) long2ip($row[\''.$k.'\']);' . "\n            ";
            } else {
                $tmp .= '$this->' . $v . ' = ('.$formData['ftypeshort'][$k].')$row[\''.$k.'\'];' . "\n        ";
                $tmp2 .= '$my'.$formData['fmodule'].'->'
                    . $v . ' = ('.$formData['ftypeshort'][$k].')$row[\''.$k.'\'];' . "\n            ";
            }
        ...
        foreach ($formData['fprop'] as $k => $v) {
            if (isset($formData['ffilterable'][$k])) {
                ...
                if ($formData['ftypeshort'][$k] == 'int') {
                    ...
                    $a = '!= \'\'';
                    $assign = "\"'\n                .Helper::unspecialtext((string) \$formData['f{$v}']).'\"";
                }
        ...
        if (count($searchabletext) > 0) {
            ...
            for ($i = 0; $i < count($searchabletext); $i++) {
                ...
                $searchabletextString .= " (\$formData['fsearchKeywordIn'] == '"
                    . $searchabletext[$i][1]."') {\n                \$whereString .= "
                    . "(\$whereString != '' ? ' AND ' : '') . '".$formData['ftablealias']."."
                    . $searchabletext[$i][0]
                    . " \n                    LIKE \'%'.\$formData['fkeywordFilter'].'%\'';\n            }";

                $combinegroup[] = "(".$formData['ftablealias']."."
                    . $searchabletext[$i][0]
                    . " \n                    LIKE \'%'.\$formData['fkeywordFilter'].'%\')";
            }
        ...
        foreach ($formData['ffilterable'] as $k => $v) {
            ...
            $tmpArgs .= "\n        \$".$prop."Filter = ".$typename."(\$_POST['".$prop."']);";
        ...
        $s['{{FILTERABLE_ARRAY}}'] = implode(', ', $tmpArray);
        ...
        if (count($formData['fsearchabletext']) > 0) {
            $tmpArgs = "\n        \$keywordFilter = Helper::plaintext"
            ...
            $tmpApply = "\n        if (strlen(\$keywordFilter) > 0) {\n"
                . $tmpApply."\n            \$formData['fkeyword'] = \$formData['fkeywordFilter'] = "
                . "\$keywordFilter;\n            \$formData['fsearchin'] = "
                . "\$formData['fsearchKeywordIn'] = \$searchKeywordIn;\n        }";
        }
        ...
        foreach ($formData['fprop'] as $k => $v) {
            if (!isset($formData['fexclude'][$k]) && $k != $formData['primaryfield']) {
                ...
                $tmp .= "\n                    \$my".$formData['fmodule']."->"
                    . $v." = $typename\$formData['f".$v."']$typenameCloseBrace;";
            }
        }
        ...
        foreach ($formData['fprop'] as $k => $v) {
            if (!isset($formData['fexcludeindex'][$k])) {
                ...
                if (isset($constantData[$k])) {
                    ...
                    $tmp .= "\n                '".$v."' => (string) \$my"
                        . $formData['fmodule']."->".$v." > 0 ? date('d/m/Y', \$my"
                        . $formData['fmodule']."->".$v.") : '-',";
                } else {
                    if ($formData['ftypeshort'][$k] == 'int') {
                        if ($v == 'ipaddress') {
        ...
        foreach ($formData['fvalidating'] as $k => $v) {
            if ($v == 'notempty') {
                $tmp .= "\n\n        if (\$formData['f"
                    . $formData['fprop'][$k]."'] == '') {\n            \$error[] = "
                    . "\$this->registry->lang['controller']['err".ucfirst($formData['fprop'][$k])
                    . "Required'];\n            \$pass = false;\n        }";
            } elseif ($v == 'greaterthanzero') {
                $tmp .= "\n\n        if (\$formData['f".$formData['fprop'][$k]
                    . "'] <= 0) {\n            \$error[] = \$this->registry->lang['controller']['err"
                    . ucfirst($formData['fprop'][$k])."MustGreaterThanZero'];\n            \$pass = false;\n        }";
            } elseif ($v == 'email') {
                $tmp .= "\n\n        if (Helper::validateEmail(\$formData['f"
                    . $formData['fprop'][$k]."'])) {\n            \$error[] = \$this->registry->lang"
                    . "['controller']['err".ucfirst($formData['fprop'][$k])
                    . "InvalidEmail'];\n            \$pass = false;\n        }";
            }
        }
        ...
        foreach ($formData['fvalidating'] as $k => $v) {
            if ($v == 'notempty') {
                $tmp .= "\n\t" . '<lines name="err'.ucfirst($formData['fprop'][$k])
        ...
        foreach ($formData['ffilterable'] as $k => $v) {
            $prop = $formData['fprop'][$k];
            $tmpControl .= '<option value="'.$prop.'">{$lang.controller.label'.ucfirst($prop).'}</option>' . "\n";
            ...
            if (isset($constantData[$k])) {
                ...
                $tmpControlMore .= '<input type="text" id="filter_'.$prop.'_more" value="{$formData.f'
        ...
        if (count($formData['fsearchabletext']) > 0) {
            ...
            foreach ($formData['fsearchabletext'] as $k => $v) {
                $prop = $formData['fprop'][$k];
                $keywordinoption .= "\n\t\t\t\t\t" . '<option value="'.$prop.'" {if $formData.fsearchin eq "'
                    . $prop.'"}selected="selected"{/if}>{$lang.controller.label'.ucfirst($prop).'}</option>';
                $tmpPlaceholder[] = '{$lang.controller.label'.ucfirst($prop).'}';
            }

            $tmpControl = '{$lang.controller.formKeywordLabel}:'."\n\t\t\t\t".
                            '<input type="text" name="fkeyword" id="fkeyword" size="20" '
                            . 'value="{$formData.fkeyword|@htmlspecialchars}" class="" />' . "\n\t\t\t\t" .
        ...
        foreach ($formData['fprop'] as $k => $v) {
            if (!isset($formData['fexclude'][$k]) && $k != $formData['primaryfield']) {
                ...
                $colMdWidth = 6;
                if ($v == 'enable') {
                    $inputstring = '<select class="input-mini" name="fenable" id="fenable">
                        <option value="1">{$lang.default.formYesLabel}</option>
                        <option value="0" {if $formData.fenable == \'0\'}selected="selected"{/if}>
                        {$lang.default.formNoLabel}</option></select>';
                } elseif (isset($constantData[$k])) {
                    //constant must used in Select box
                    $inputstring = '<select class="" name="f' . $v . '" id="f'
                        . $v.'">{html_options options=$'.$v.'Options selected=$formData.f'.$v.'}</select>';
                } elseif ($this->getDefaultValueFromColumnType($formData['ftypeshort'][$k]) === 0) {
                    $inputstring = '<input type="text" name="f'.$v.'" id="f'
                        . $v.'" value="{$formData.f'.$v.'|@htmlspecialchars}" class="form-control">';
                } else {
                    //detect varchar 512
                    if (strpos($formData['ftype'][$k], 'text') !== false
                        || preg_replace('/[^0-9]/', '', $formData['ftype'][$k]) > 255) {
                        $colMdWidth = 12;
                        $inputstring = '<textarea name="f'.$v.'" id="f'
                            . $v.'" rows="5" class="">{$formData.f'.$v.'}</textarea>';
                    } else {
                        $inputstring = '<input type="text" name="f'.$v.'" id="f'
                            . $v.'" value="{$formData.f'.$v.'|@htmlspecialchars}" class="form-control">';
                    }
                ...
                $tmp .= "\n\n\t".'<div class="col-md-'.$colMdWidth.' ssb clear inner-left">'."\n\t\t"
                    . '<label for="f'.$v.'">{$lang.controller.label'.ucfirst($formData['fprop'][$k]).'}'
                    . $requireString.'</label>'."\n\t\t".''.$inputstring.''."\n\t".'</div>';
            }
        }
    ...
    private function getDefaultValueFromColumnType($typeshort)
    {
        ...
        if ($typeshort == 'int' || $typeshort == 'float'
            || $typeshort == 'double' || $typeshort == 'decimal' || $typeshort == 'real') {
            $defaultValue = 0;
        }
