...
class ImageResizer
{
    ...
    private $watermarkPadding = array(
        'top' => 0,
        'right' => 0,
        'bottom' => 0,
        'left' => 0
     ...
     *
     * @param string $sf: sourceFolder : end with slash '/'
     * @param string $sn : sourceName
     * @param string $df : destFolder   : end with slash '/'
     * @param string $dn : destName
     * @param int    $mw : maxWidth
     * @param int    $mh : maxHeight
     ...
     * @return ImageResizer
     */
    public function __construct($sf, $sn, $df, $dn, $mw = 0, $mh = 0, $cropRatio = '', $quality = 90, $color = '')
    {
        $this->sourceFolder = $sf;
        $this->sourceName = $sn;
        $this->destFolder = $df;
        $this->destName = $dn;
        $this->maxWidth = $mw;
    ...
    public function addWatermark($watermarkFile, $watermarkPosition = '', $watermarkPadding = '')
    {
        ...
        if (in_array($watermarkPosition, array(
            'center',
            'topleft',
            'topcenter',
            'topright',
        ...
        } elseif (strlen($watermarkPadding) > 0) {
            $paddingArray = split(',', $watermarkPadding);
            if (isset($paddingArray[0])) {
                $this->watermarkPadding['top'] = (int) ($paddingArray[0]);
            }
            if (isset($paddingArray[1])) {
                $this->watermarkPadding['right'] = (int) ($paddingArray[1]);
            }
            if (isset($paddingArray[2])) {
                $this->watermarkPadding['bottom'] = (int) ($paddingArray[2]);
            }
            if (isset($paddingArray[3])) {
                $this->watermarkPadding['left'] = (int) ($paddingArray[3]);
            }
        }
    }
    /**
     ...
     *
     * @param array $cornerImages : assoc array (
     * 'topleft' => PNG image, 'topright' => PNG image, 'bottomright' => PNG image, 'bottomleft' => PNG image)
     */
     ...
     * Output the image
     *
     * @param boolean $save : If true: save image to selected destFolder and destName,
     * otherwise, output image to browser with encoding of image
     */
    public function output($save = true)
    {
        ...
        if (((!$this->maxWidth && !$this->maxHeight)
            || ($this->maxWidth >= $width && $this->maxHeight >= $height)) && $this->watermarkFile == '') {
            if ($save) {
            ...
            if ($xRatio > 1 && $yRatio > 1) {
                ...
                if ($xRatio * $height < $this->maxHeight) { // Resize the image based on width
            ...
            switch ($this->size['mime']) {
                case 'image/gif':
                    ...
                    $doSharpen = false;
                    // We are converting the GIF to a PNG and PNG
                    // needs a compression level of 0 (no compression) through 9
            ...
            if (in_array($this->size['mime'], array(
                'image/gif',
                'image/png'
            ))) {
                if (!$color) {
            ...
            if ($this->watermarkFile != '' && file_exists($this->watermarkFile)) {
                ...
                switch ($this->watermarkPosition) {
                    case 'center':
                        $dst_x+= (int) ($tnWidth / 2 - $watermarkWidth / 2);
