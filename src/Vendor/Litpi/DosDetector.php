...
class Dosdetector
{
    ...
    public function run($landingpage = '')
    {
        ...
        // But, because the expire of key is 3600 (1 hour), so,
        // there are max 2 key in mean time (because old keys will be deleted because cache expireds ^^)
        $dateInfo = getdate();
        ...
        if ($this->isMemoryCacheEnable()) {
            ...
            $useragent = isset($_SERVER['HTTP_USER_AGENT']) ? strip_tags(substr(
            ...
            if (!in_array($ipaddress, $this->ignoreIpAddress)) {
                ...
                if (!$accessList) {
                    $accessList = array(
                        $ipaddress => array(
                            $curTime => array(1, $havecookie, $useragent, $uri, $referer)
                        )
                    );
                } else {
                    //Neu may da truy cap
    ...
    {
        return ( (extension_loaded('apc') && ini_get('apc.enabled') ) || extension_loaded('memcached'));
    }
