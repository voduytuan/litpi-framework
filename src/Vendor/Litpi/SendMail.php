...
class SendMail
{
    ...
    public function __construct($registry, $toEmail, $toName, $subj, $content, $fromEmail = '', $fromName = '')
    {
        $this->registry = $registry;
        $this->fromEmail = $fromEmail;
        $this->fromName = $fromName;
        $this->mailSubject = $subj;
        $this->mailContents = $content;
    ...
    public function send()
    {
        ...
        if ($this->registry->setting['mail']['usingAmazonses']) {
            ...
            if ($this->usingSMTP) {
                ...
                if (is_array($this->registry->conf['smtp']['host'])
                    && is_array($this->registry->conf['smtp']['username'])
                    && is_array($this->registry->conf['smtp']['password'])) {
    ...
    public function preview()
    {
        echo '<div style="background:#fff;margin:20px;padding:10px;line-height:1.5;">
            <div>
                ...
                . $this->fromName.' <span style="color:#999;">&lt;'.$this->fromEmail.'&gt;</span></div>
