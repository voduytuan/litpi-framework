...
class ViephpHashing
{
     ...
     * @param  boolean $full  : if $full == true : create full hashing string: used to create password;
     * if $full == false: only return password hashing prefix(used for remember me to save cookie)
     * @return unknown
     */
    public static function hash($input, $full = true)
    {
        if ($full) {
            $output = md5($input . self::$salt)
                . self::$saltSeperator . base64_encode(md5(self::$secretString) . self::$salt);
            return $output;
        } else {
     ...
     * @param string $dest   : password duoc luu tru trong db, co format;
     * {HASH}{SALT_SEPERATOR}{ENCODEDSALT} (HASH = MD5({password}{$SALT}),
     * ENCODEDSALT = BASE64ENCODE(MD5(SECRETSTRING).{$SALT}))
     */
    ...
    {
        $output = self::superBase64Encode($userid)
            . self::$saltSeperator . self::superBase64Encode(self::hash($password, false));
        return $output;
    }
