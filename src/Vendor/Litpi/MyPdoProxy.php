...
class MyPdoProxy
{
        ...
        if (!isset($this->isconnected[$replicate . '-' . $identifier])) {
            try {
                if ($isMaster) {
                    ...
                    $pdodriver = 'mysql:host=' . $this->connectinfo['master'][$identifier]['host']
                    ...
                    $pdodriver = 'mysql:host=' . $this->connectinfo['slave'][$identifier]['host'] . ';dbname='
                        . $this->connectinfo['slave'][$identifier]['database'];
                ...
                if (isset($_COOKIE['isdeveloper']) || isset($_COOKIE['isadministrator'])) {
                    ...
                    die('Database connection failed. ('
                        . strtoupper($replicate)
                        . ': '. $identifier . ' | '
                        . $e->getMessage(). ').
                        (Only administrator & developer can see this error message.
                        Other people will see <a href="'
                        . $registry->conf['rooturl'].'dberror.html">dberror.html</a> error page.)');
                } else {
    ...
    /**
     * Base on the SQL query, we will route the query to valid/suitable replicate(master or slave)
     * to load balancing between db servers
     */
